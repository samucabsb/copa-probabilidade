...
export function RecentMatches({ title, data }) {
  ...
  return (
    <Panel title={title} badge={`${data?.dataQuality ?? '—'} · ${data?.matchCount ?? 0} jogos`}>
      ...
      {matches.slice(0, 6).map((match, index) => (
        <div key={`${match.date}-${index}`} className="mb-3 flex items-center justify-between gap-4 rounded-2xl bg-slate-50 px-3 py-3 text-sm">
          <div><div className="font-black text-slate-800">{match.homeTeam} {match.homeScore}–{match.awayScore} {match.awayTeam}</div><div className="mt-0.5 flex items-center gap-1 text-xs text-slate-500"><CalendarDays className="h-3.5 w-3.5" />{formatDate(match.date) || 'sem data'} · {match.competition}</div></div>
